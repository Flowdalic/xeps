...
It is **REQUIRED** that the application-protocol specific extension provides a mechanism to request a SASL-HT token.
...
It is **RECOMMENDED** that the the protocol allows the requestor to signal the name of the HT mechanism which he intends to use with the token.
If a token is used with a different mechanism than the one which was signaled upon requesting the token, then the authentication **MUST** fail.
This allows to pin the token to a the SASL mechanism, which increases the security because it makes it impossible for an attacker to downgrade the SASL mechanism.
